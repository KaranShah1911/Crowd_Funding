...
const CampaignDetails = () => {
  ...
  const defaultCampaign = {
    image: 'default-image.png', // Replace with a default image
    owner: '0x1234567890abcdef',
    description: 'Default campaign description',
    target: '10 ETH',
    amountCollected: '0 ETH',
    deadline: '0 Days Left',
    avatar: 'default-avatar.png', // Replace with a default avatar
  };

  // Use the passed campaign data or fallback to default
  const campaign = state?.campaign || defaultCampaign;
  const donators = state?.donators || [];
    ...
    <div>
      <div className="w-full flex md:flex-row flex-col mt-10 gap-[30px]">
        <div className="flex-1 flex-col">
          ...
          <div className="relative w-full h-[5px] bg-[#3a3a43] mt-2">
            <div className="absolute h-full bg-[#4acd8d]" style={{ width: `45%`, maxWidth: '100%' }}></div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="mt-[20px] flex flex-row items-center flex-wrap gap-[14px]">
              <div className="w-[52px] h-[52px] flex items-center justify-center rounded-full bg-[#2c2f32]">
                <img src={campaign.avatar} alt="user" className="w-[60%] h-[60%] object-contain" />
              </div>
              <div>
                <h4 className="font-epilogue font-semibold text-[14px] text-white">{campaign.owner}</h4>
                <p className="mt-[4px] font-epilogue font-normal text-[12px] text-[#808191]">10 Campaigns</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-[20px] flex flex-col p-4 bg-[#1c1c24] rounded-[10px]">
            ...
            <div className="mt-[30px]">
              <input
                ...
                placeholder="ETH 0.1"
                step="0.01"
                className="w-full py-[10px] px-[15px] border-[1px] border-[#3a3a43] bg-transparent text-white text-[18px] placeholder:text-[#4b5264] rounded-[10px]"
                value={donationAmount}
                onChange={(e) => setDonationAmount(e.target.value)}
              />
              <div className="my-[20px] p-4 bg-[#13131a] rounded-[10px]">
                <h4 className="font-epilogue font-semibold text-[14px] text-white">Back it because you believe in it.</h4>
                <p className="mt-[20px] font-epilogue font-normal text-[#808191]">Support the project for no reward, just because it speaks to you.</p>
              </div>
              <CustomButton
                btnType="button"
                title="Fund Campaign"
                styles="w-full bg-[#8c6dfd]"
                handleClick={() => handleDonate(campaign.id, donationAmount)}
              />
            </div>
          </div>
        </div>
